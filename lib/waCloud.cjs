...
const SEND_ERRORS = {
  ...
  // Calling API
  131044: { status: 409, error: 'Meta exige un método de pago activo en la cuenta de WhatsApp Business para las llamadas (se cobran por minuto). Agrégalo en WhatsApp Manager → Configuración → Métodos de pago y reintenta.' },
  138018: { status: 409, error: 'Meta no permite habilitar llamadas hasta que el webhook esté suscrito al campo "calls".' },
