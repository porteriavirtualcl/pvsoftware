          ...
          {filteredTechs.map((tech) => (
            <motion.div layout key={tech.id} className="relative group bg-gray-900 border border-gray-800 rounded-[2.5rem] p-10 hover:border-blue-500/50 transition-all duration-500 overflow-hidden">
               <div className="absolute top-0 right-0 p-10 flex gap-2">
                  <span className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest ${tech.status === 'active' ? 'bg-green-500/10 text-green-500' : 'bg-gray-800 text-gray-500'}`}>
                    {tech.status}
                  </span>
               </div>
               ...
               <div className="flex items-center gap-6 mb-8">
                  ...
                  <div className="space-y-1">
                     <h3 className="text-2xl font-black text-white uppercase italic tracking-tight">{tech.name}</h3>
                     <p className="text-xs text-blue-500 font-black uppercase tracking-[0.2em]">{tech.specialty}</p>
                  </div>
               </div>

               <div className="space-y-4 mb-8">
                  <div className="flex items-center gap-3 text-sm text-gray-400 font-medium"><Phone size={14} className="text-blue-500" /> {tech.phone}</div>
                  <div className="flex items-center gap-3 text-sm text-gray-400 font-medium"><Mail size={14} className="text-blue-500" /> {tech.email}</div>
                  <div className="flex flex-wrap gap-2 pt-4">
                     {tech.assignment.includes('all') ? (
                       <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase text-gray-500 tracking-widest italic">Global Mastery Access</span>
                     ) : (
                       tech.assignment.map(id => (
                          <span key={id} className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase text-gray-500 tracking-widest italic">{condos.find(c => c.id === id)?.name || 'Condominio'}</span>
                       ))
                     )}
                  </div>
               </div>
               ...
               <div className="flex items-center justify-between pt-8 border-t border-gray-800">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-[10px] font-black text-gray-500 uppercase">Red Sincronizada</span>
                  </div>
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
             ...
             <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="relative w-full max-w-xl bg-gray-900 border border-gray-800 rounded-[3rem] p-10 overflow-y-auto max-h-[90vh] no-scrollbar shadow-[0_0_100px_rgba(59,130,246,0.1)]">
                <h3 className="text-3xl font-black text-white italic uppercase tracking-tight mb-10">Ficha Técnica</h3>
                <form onSubmit={handleSave} className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Nombre Completo Specialist</label>
                   ...
                   <div className="grid grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Especialidad / Firma</label>
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Asignación de Condominios</label>
                      <div className="p-6 bg-gray-950 border border-gray-800 rounded-2xl space-y-3 max-h-40 overflow-y-auto no-scrollbar">
                         <label className="flex items-center gap-3 cursor-pointer group">
                            ...
                            <span className="text-sm font-black text-gray-500 group-hover:text-white uppercase italic tracking-widest transition-colors">Maestría Global (Todos)</span>
                         </label>
                         {condos.map(c => (
