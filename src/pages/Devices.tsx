     ...
     <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 flex items-center gap-4">
          ...
          <div><p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">En Línea</p><p className="text-2xl font-black text-white">{devices.filter(d => d.status === 'online').length}</p></div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 flex items-center gap-4">
          <div className="w-12 h-12 bg-red-500/10 rounded-2xl flex items-center justify-center text-red-500"><WifiOff size={24} /></div>
          <div><p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Desconectados</p><p className="text-2xl font-black text-white">{devices.filter(d => d.status === 'offline').length}</p></div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 flex items-center gap-4">
          <div className="w-12 h-12 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-500"><Activity size={24} /></div>
          <div><p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Eventos/Hoy</p><p className="text-2xl font-black text-white">1,248</p></div>
        </div>
        <button onClick={handleOpenAdd} className="bg-blue-600 hover:bg-blue-500 rounded-3xl p-6 flex items-center justify-center gap-4 transition-all group">
          ...
          <span className="text-white font-black uppercase tracking-widest text-sm">Añadir Hardware</span>
        </button>
      </div>
           ...
           <motion.div layout key={device.id} className="bg-gray-900 border border-gray-800 rounded-[2.5rem] p-8 relative overflow-hidden group">
              ...
              <div className="flex items-start gap-6 mb-8">
                 ...
                 <div className="space-y-1">
                    <h3 className="text-2xl font-black text-white italic">{device.name}</h3>
                    <p className="text-blue-500 font-bold text-xs uppercase tracking-[0.2em]">{device.condoName}</p>
                    <div className="flex items-center gap-2 mt-2">
                       <Globe size={12} className="text-gray-600" />
                       <span className="text-xs font-mono text-gray-500">{device.ipAddress}:{device.port}</span>
                    </div>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                 <div className="bg-gray-950/50 p-4 rounded-2xl border border-gray-800/50">
                    <p className="text-[9px] font-black text-gray-600 uppercase mb-2">LPR Tracking</p>
                    <div className={`w-3 h-3 rounded-full ${device.capturedEvents.lpr ? 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]' : 'bg-gray-800'}`} />
                 </div>
                 <div className="bg-gray-950/50 p-4 rounded-2xl border border-gray-800/50">
                    <p className="text-[9px] font-black text-gray-600 uppercase mb-2">Face Recon</p>
                 ...
                 <div className="bg-gray-950/50 p-4 rounded-2xl border border-gray-800/50">
                    <p className="text-[9px] font-black text-gray-600 uppercase mb-2">QR Gates</p>
              ...
              <div className="flex items-center justify-between pt-8 border-t border-gray-800">
                <div className="flex items-center gap-4">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">API Bridge OK</span>
                   </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
             ...
             <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-2xl bg-gray-900 border border-gray-800 rounded-[3rem] p-10 overflow-y-auto max-h-[90vh] no-scrollbar shadow-[0_0_100px_rgba(59,130,246,0.1)]">
                <div className="flex justify-between items-center mb-10">
                   <div>
                      <h3 className="text-3xl font-black text-white italic">{editingDevice ? 'Configurar Hardware' : 'Integrar Nuevo Dispositivo'}</h3>
                      <p className="text-gray-500 text-sm mt-1">Sincroniza cámaras LPR, paneles faciales y barreras.</p>
                   </div>
                ...
                <form onSubmit={handleSave} className="space-y-8">
                   <div className="grid grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1">Identificador Dispositivo</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1">Firma de Hardware</label>
                   ...
                   <div className="bg-gray-950/50 p-8 rounded-3xl border border-gray-800 space-y-6">
                      ...
                      <div className="grid grid-cols-3 gap-6">
                         <div className="col-span-2 space-y-1">
                            <p className="text-[9px] font-black text-gray-600 uppercase italic">Dirección IP (LAN)</p>
                            <input required type="text" value={formData.ipAddress} onChange={(e) => setFormData({...formData, ipAddress: e.target.value})} className="w-full bg-gray-900 border border-gray-800 rounded-xl py-3 px-4 text-white font-mono text-sm focus:border-blue-600 outline-none" placeholder="192.168.1.108" />
                         </div>
                         <div className="space-y-1">
                            <p className="text-[9px] font-black text-gray-600 uppercase italic">Puerto</p>
                            <input required type="text" value={formData.port} onChange={(e) => setFormData({...formData, port: e.target.value})} className="w-full bg-gray-900 border border-gray-800 rounded-xl py-3 px-4 text-white font-mono text-sm focus:border-blue-600 outline-none" placeholder="80" />
                         </div>
                      </div>
                      <div className="space-y-1">
                         <p className="text-[9px] font-black text-gray-600 uppercase italic">DDNS / IP Pública (Acceso Remoto)</p>
                         <input type="text" value={formData.externalIp} onChange={(e) => setFormData({...formData, externalIp: e.target.value})} className="w-full bg-gray-900 border border-gray-800 rounded-xl py-3 px-4 text-white font-mono text-sm focus:border-blue-600 outline-none" placeholder="porteria.ddns.net" />
                      </div>
                   </div>

                   <div className="grid grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Usuario Hardware</label>
                   ...
                   <div className="space-y-4">
                      <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mb-4">Sensores e Inteligencia Artificial</p>
                      <div className="flex flex-wrap gap-6">
                        <label className="flex items-center gap-3 cursor-pointer group">
                           ...
                           <span className="text-sm font-bold text-gray-400 group-hover:text-white transition-colors">Lectura de Patentes</span>
                        </label>
                        <label className="flex items-center gap-3 cursor-pointer group">
                           ...
                           <span className="text-sm font-bold text-gray-400 group-hover:text-white transition-colors">Acceso Facial</span>
                        </label>
                        <label className="flex items-center gap-3 cursor-pointer group">
                           ...
                           <span className="text-sm font-bold text-gray-400 group-hover:text-white transition-colors">Lectura de Pases QR</span>
                        </label>
                      </div>
                   </div>
        ...
        {deletingDevice && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
             ...
             <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-sm bg-gray-900 border border-gray-800 rounded-[2.5rem] p-10 text-center shadow-2xl">
                ...
                <h3 className="text-2xl font-black text-white italic mb-2">¿Eliminar Hardware?</h3>
                <p className="text-gray-500 text-sm font-medium mb-10">La desconexión de <span className="text-white font-bold">{deletingDevice.name}</span> interrumpirá el flujo de eventos LPR/Facial.</p>
